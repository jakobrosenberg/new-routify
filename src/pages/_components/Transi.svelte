  ...
  function getTop(node, y) {
    return {
      update(y) {
        ...
        if (view >= 0 && view <= threshold) {
          opacity = view / threshold;
        }
      },
      destroy() {}
    };
  ...
  function getSide(node, y) {
    return {
      update(y) {
        ...
        if (scopeOfEntry >= 0 && scopeOfEntry <= threshold) {
          side = from - (scopeOfEntry * from) / threshold;
        } else if (scopeOfEntry > threshold) {
          side = 0;
        } else {
          side = from;
        }
